...
int main(int argc, char *argv[]) {
    ...
    program.add_argument("-c", "--config")  // 相机配置文件
        .help("Configuration YAML file.")
        .nargs(1);
    program.add_argument("--csv").help("Save CSV-format trajectory.").nargs(1);  // csv 格式的轨迹输出
    program.add_argument("--tum").help("Save TUM-format trajectory.").nargs(1);  // tum 格式的轨迹输出
    program.add_argument("-p", "--play")  // 是否立即播放
        .help("Start playing immediately.")
        .default_value(false)
        .implicit_value(true);
    program.add_argument("-H", "--headless")  // 无 UI 模式
        .help("Don't show visualization, implies -P.")
        .default_value(false)
        .implicit_value(true);
    program.add_argument("input").help("input file");  // 数据集位置
    ...
    std::shared_ptr<xrslam::extra::YamlConfig> yaml_config =
        std::make_shared<xrslam::extra::YamlConfig>(config); // 读完 euroc.yaml 中的内容
    std::unique_ptr<xrslam::XRSLAM> xrslam =
        std::make_unique<xrslam::XRSLAM>(yaml_config);  // 若使用多线程，则运行 SLAM；单线程只调整运行标志

    std::cout << "input: " << input << std::endl;
    std::unique_ptr<DatasetReader> reader = DatasetReader::create_reader(input); // 读完数据集中的图像和 IMU 数据（图像以路径形式体现）
    if (!reader) {
        fprintf(stderr, "Cannot open \"%s\"\n", input.c_str());
        return EXIT_FAILURE;
    }

    // 初始化输出的类（输出可选格式：csv 或 tum）
    std::vector<std::unique_ptr<TrajectoryWriter>> outputs;
