...
void Frame::detect_keypoints(Config *config) {
    ...
    image->detect_keypoints(pkeypoints,
                            config->feature_tracker_max_keypoint_detection()/* 150 */,
                            config->feature_tracker_min_keypoint_distance()/* 20 */);
...
void Frame::track_keypoints(Frame *next_frame, Config *config) {
    ...
    // 基于本质矩阵得到旋转矩阵，然后计算得到70%处的视差角，存入 feature_tracker_angle_misalignment
