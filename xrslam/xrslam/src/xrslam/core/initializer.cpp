...
void Initializer::mirror_keyframe_map(Map *feature_tracking_map,
    ...
    size_t init_frame_index_last =
        feature_tracking_map->frame_index_by_id(init_frame_id); // 在所有帧中的顺序
    ...
    // 直到积累了 35 帧之后才进行初始化
    if (init_frame_index_last < init_frame_index_distance/* 35 */) {
        map.reset();
        return;
    }
    ...
    // 初始化 Initializer 中的 Map，添加上述 Frame 进去
            ...
            if (Track *track = old_frame_i->get_track(ki)) {
                if (size_t kj = track->get_keypoint_index(old_frame_j);
                    ...
                    new_frame_i->get_track(ki, nullptr)
                        ->add_keypoint(new_frame_j, kj);  // ki <-> kj 间建立了一个 track
                }
            }
        }

        // 由于是间隔 5 帧一个初始化关键帧，需要累积中间的 IMU 数据
...
std::unique_ptr<SlidingWindowTracker> Initializer::initialize() {
    if (!map)
        return nullptr;
    if (!init_sfm())  // 所有初始化帧之间进行一次 SfM（整个过程类似 VINS-mono 的相应部分）
        return nullptr;
    if (!init_imu())
        return nullptr;
    ...
    for (size_t i = 0; i < map->frame_num(); ++i) {
        ...
        for (size_t j = 0; j < frame->keypoint_num(); ++j) {
            ...
            solver->add_track_states(track); // 逆深度
        }
    }
    for (size_t i = 0; i < map->frame_num(); ++i) {
        Frame *frame = map->get_frame(i);
        for (size_t j = 0; j < frame->keypoint_num(); ++j) {
            ...
            if (frame == track->first_frame())
                continue;
            solver->add_factor(frame->reprojection_error_factors[j].get());  // 重投影误差
        }
    }
    for (size_t j = 1; j < map->frame_num(); ++j) {
        ...
        if (frame_j->preintegration.integrate(frame_j->image->t,
            ...
            solver->put_factor(Solver::create_preintegration_error_factor(
                frame_i, frame_j, frame_j->preintegration)); // 预积分因子
        }
    }
    solver->solve();
...
bool Initializer::init_sfm() {
    ...
    std::vector<std::pair<size_t, size_t>> init_matches; // 特征点 ID 的匹配
    std::vector<char> init_point_status;
    std::vector<vector<2>> frame_i_keypoints; // 归一化坐标
    std::vector<vector<2>> frame_j_keypoints; // 归一化坐标
    matrix<3> init_R;
    vector<3> init_T;

    // 首尾两帧的 track，至少要建立 50 个 track，且每个角点的平均光流模长至少为 10
    ...
    matrix<3> H = find_homography_matrix(frame_i_keypoints, frame_j_keypoints,
                                         ...
                                         1000, config->random()); // 求 H 矩阵
    if (!decompose_homography(H, RH1, RH2, TH1, TH2, nH1, nH2)) {  // 分解求位姿，不能有纯旋转
    ...
    // [1.1] triangulation
    // 从多解中选择一个最佳解：优先三角化数量最多，其次是三角化后的重投影误差最小
    ...
    // [2.1] set init states
    // 相机位姿：第一帧 body 系下位姿设为零，通过外参可得到相机位姿
    PoseState pose;
    pose.q.setIdentity();
    pose.p.setZero();
    ...
    for (size_t k = 0; k < init_points.size(); ++k) {
        ...
        track->set_landmark_point(init_points[k]); // 设置逆深度
