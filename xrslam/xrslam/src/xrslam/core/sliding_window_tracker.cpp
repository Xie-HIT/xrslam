...
void SlidingWindowTracker::mirror_frame(Map *feature_tracking_map,
    ...
    Frame *old_frame_i = feature_tracking_map->get_frame(frame_index_i); // 之前 SlidingWindowTracker 中的最后一帧
    Frame *old_frame_j = feature_tracking_map->get_frame(frame_index_j); // 图像队列的最后一帧
    ...
    map->attach_frame(curr_frame->clone()); // 将图像队列的最后一帧插入到 SlidingWindowTracker 的 Map 中
        ...
        if (Track *track = newframe_j->get_track(k)) {
            if (!track->tag(TT_TRIANGULATED)) {
                if (auto p = track->triangulate()) {
                    track->set_landmark_point(p.value()); // 设置深度的初始值
...
void SlidingWindowTracker::refine_window() {
    ...
    for (size_t i = 0; i < map->frame_num(); ++i) {
        ...
        for (size_t j = 0; j < frame->keypoint_num(); ++j) {
            ...
            if (!track->first_frame()->tag(FT_KEYFRAME))
                continue;
            solver->add_track_states(track); // 设置逆深度为优化状态
        }
    }
    ...
    for (size_t k = 0; k < map->track_num(); ++k) {
        ...
        if (track->tag(TT_TRIANGULATED)) {
            ...
            is_valid = is_valid && (rpe / std::max(rpe_count, 1.0) < 3.0); // 优化后重投影误差需小于 3
    ...
    map->prune_tracks([](const Track *track) { return !track->tag(TT_VALID); }); // 丢掉外点
}
...
void SlidingWindowTracker::refine_subwindow() {
    Frame *frame = map->get_frame(map->frame_num() - 1); // 获取最新的关键帧
    if (frame->subframes.empty()) {
        return;
    }
    if (frame->subframes[0]->tag(FT_NO_TRANSLATION)) {
        // subframes 过多时，为了效率考虑，只取 1/3 的 subframes
