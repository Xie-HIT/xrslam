...
void FeatureTracker::work(std::unique_lock<std::mutex> &l) {
    ...
    bool sliding_window_frame_tag =
        !is_initialized ||
        frame->id() % config->sliding_window_tracker_frequent() == 0; // always true in current setting
    synchronized(map) {
        // TODO：一开始 map->frame_num() 为零，之后逐渐增加到 50 多，最后突然减小并稳定到 20
        if (map->frame_num() > 0) {
            // 若已经初始化，则意味着已经存在优化好的关键帧了，基于 IMU 信息更新该帧之后的所有帧的位姿
            if (is_initialized) {
            ...
            // 该部分在 IOS 时用到
            if (is_initialized) {
                frame->preintegration.predict(last_frame, frame.get()); // 基于 IMU 信息得到当前帧的位姿初始值
#if defined(XRSLAM_IOS)
                synchronized(keymap) {
                    attach_latest_frame(frame.get()); // 加入到 keymap 中
                    solve_pnp(); // 基于重投影误差 BA 求解 PnP
                    Frame *latest_frame =
        ...
        // TODO：若地图中帧过多，且第一帧不是最近一次被优化的帧，则删除地图的第一帧
        while (map->frame_num() >
                   (is_initialized
        ...
        inspect_debug(feature_tracker_painter, p) {
            if (p.has_value()) {
                ...
                painter->set_image(frame->image.get()); // 将灰度图转化成 BGR 彩图，并保存在 painter 里的 canvas 成员中
                for (size_t i = 0; i < frame->keypoint_num(); ++i) {
                    if (Track *track = frame->get_track(i)) {
                        color3b c = {0, 255, 0}; // 跟踪上的特征点为绿色
                        painter->point(apply_k(frame->get_keypoint(i), frame->K)
                                           .cast<int>(),
                                       c, 5);
                    } else {
                        color3b c = {255, 0, 255}; // 未跟踪上的特征点为粉色
                        painter->point(apply_k(frame->get_keypoint(i), frame->K)
                                           .cast<int>(),
                                       c, 5, 1);
                    }
