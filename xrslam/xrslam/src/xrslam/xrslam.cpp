...
XRSLAM::XRSLAM(std::shared_ptr<Config> config) {
    log_message(XRSLAM_LOG_INFO, (char *)logo_ascii, XRSLAM_VERSION_STRING); // 打印 LOGO
    config->log_config(); // 打印 config 内容，并输出 XRSLAM_ENABLE_THREADING 的状态

    // 启动 frontend_worker 和 feature_tracker 两个线程
    // 但如果不是多线程模式，则只调整 flag
