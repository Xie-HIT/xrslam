...
static YAML::Node find_node(const YAML::Node &root, const std::string &path,
    ...
    while (std::getline(ss, child, '.')) {
        node.reset(node[child]); // navigate through the tree without changing the input data
    }
    if (!node) {
        if (mandatory) {
...
YamlConfig::YamlConfig(const std::string &filename) {
    ...
    m_sliding_window_size = Config::sliding_window_size();  // 滑窗长度：10
    ...
    m_feature_tracker_max_keypoint_detection =
        Config::feature_tracker_max_keypoint_detection(); // 跟踪时检测特征点的最大数目：150
    ...
    m_initializer_keyframe_num = Config::initializer_keyframe_num();  // 初始化时关键帧的数目：8
    ...
    m_solver_iteration_limit = Config::solver_iteration_limit(); // 优化迭代的最大次数：10
    ...
    if (auto node = find_node(config, "sliding_window.size", false)) {
        assign(m_sliding_window_size, node);
    }

    if (auto node = find_node(config, "sliding_window.force_keyframe_landmarks",
                              false)) {
        assign(m_sliding_window_force_keyframe_landmarks, node);
    }

    if (auto node =
            find_node(config, "feature_tracker.min_keypoint_distance", false)) {
        assign(m_feature_tracker_min_keypoint_distance, node);
    }

    if (auto node = find_node(config, "feature_tracker.max_keypoint_detection",
                              false)) {
        assign(m_feature_tracker_max_keypoint_detection, node);
    }

    if (auto node =
            find_node(config, "feature_tracker.max_init_frames", false)) {
    ...
    if (auto node =
            find_node(config, "feature_tracker.predict_keypoints", false)) {
        assign(m_feature_tracker_predict_keypoints, node);
    }

    if (auto node = find_node(config, "initializer.keyframe_num", false)) {
    ...
    // 是否启用视觉定位
    if (auto node = find_node(config, "visual_localization_enable", false)) {
